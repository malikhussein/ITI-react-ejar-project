...
const ProductDetails = () => {
  ...
  />
      ...
      <div className="d-flex align-items-center mb-2">
        ...
        <span className="ms-1 text-muted">{fields.category || "N/A"}</span>
      </div>

      <div className="d-flex align-items-center mb-2">
        <i className="fa-solid fa-user me-2 text-secondary"></i>
        <span className="fw-semibold">Owner:</span>
        <Link to={`/profile/${fields.ownerId}`} className="ms-1 text-decoration-none text-primary fw-semibold">
          {fields.owner}
        </Link>
      </div>
                ...
                <p className="text-muted">
                  {Array.from({ length: 5 }, (_, i) => (
                    <i
                      key={i}
                      className={`fa-star ${
                        i + 1 <= Number(product.data.averageRating) ? 'fas' : 'far'
                      }`}
